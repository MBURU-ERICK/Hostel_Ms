...
<head>
    ...
    <title>Hostels Management - HostelHub Admin</title>
...
<body class="bg-gray-100">
        ...
        <div class="flex-1 p-8">
            ...
            <div class="mb-8">
                <div class="flex justify-between items-center">
                    <div>
                        <h1 class="text-3xl font-bold text-gray-900">Hostels Management</h1>
                        <p class="text-gray-600 mt-2">Manage all hostel listings in the system</p>
                    </div>
                </div>
            </div>
            ...
            <div class="bg-white rounded-lg shadow-sm border border-gray-200">
                <div class="p-6 border-b border-gray-200">
                    <h3 class="text-lg font-semibold text-gray-900">All Hostels</h3>
                </div>
                <div class="p-6">
                    @if($hostels->count() > 0)
                            ...
                            <table class="w-full">
                                <thead>
                                    <tr class="border-b border-gray-200">
                                        <th class="text-left py-3 px-4 text-sm font-medium text-gray-600">Hostel</th>
                                        <th class="text-left py-3 px-4 text-sm font-medium text-gray-600">Landlord</th>
                                        ...
                                        <th class="text-left py-3 px-4 text-sm font-medium text-gray-600">Reviews</th>
                                ...
                                <tbody>
                                    @foreach($hostels as $hostel)
                                        <tr class="border-b border-gray-100 hover:bg-gray-50">
                                            <td class="py-4 px-4">
                                                <div>
                                                    <p class="font-medium text-gray-900">{{ $hostel->name }}</p>
                                                    <p class="text-sm text-gray-500">{{ $hostel->location }}</p>
                                                </div>
                                            </td>
                                            <td class="py-4 px-4">
                                                <p class="text-sm text-gray-900">{{ $hostel->landlord->name }}</p>
                                                <p class="text-xs text-gray-500">{{ $hostel->landlord->email }}</p>
                                            </td>
                                            <td class="py-4 px-4 text-sm font-semibold text-green-600">
                                                Ksh {{ number_format($hostel->rent_per_month, 2) }}
                                            </td>
                                            ...
                                            <td class="py-4 px-4">
                                                <span class="text-xs px-2 py-1 rounded-full
                                                    {{ $hostel->is_available ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800' }}">
    ...
    {{ $hostel->is_approved ? 'Approved' : 'Pending Approval' }}
                                            ...
                                            <td class="py-4 px-4">
                                                <a href="{{ route('admin.hostels.show', $hostel->id) }}"
                    ...
                    @else
                        <p class="text-gray-500 text-center py-8">No hostels found.</p>
                    @endif
                </div>
            </div>
        </div>
